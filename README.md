...
> **Current deployment scope (30 July 2026):** ARIA is deployed only in Zone A using ESP-A1, ESP-A2 and one DJI Osmo Action 5 Pro stream (`camera_a`). Zone B, Zone T, ESP-B, ESP-T and the second camera are retired from live collection. Historical implementation and test records are retained in the repository for traceability and are not presented as the active configuration.
...
> **Previous project status — completed 26 July 2026:**  
> Individual ESP-A, ESP-B, and ESP-T hardware builds were completed and tested independently. The final three-node hardware split, node-specific firmware, Wi-Fi UDP telemetry, sensor wiring, and hardware documentation were established.

> **Current project status — 20 August 2026:** Phases 9 and 10 are complete, and participant collection was permanently closed after the ninth retained formal session. Ethics Pack Version 4.4 was generated, visually verified, issued to participating Counter Agents and acknowledged before collection. The required live checks were passed by ESP-A1, ESP-A2 and wireless DJI Camera A, and final staff-side placements were documented and maintained in a physically safe condition throughout retained sessions.
> 
> Phase 2.3 calibration and the versioned Camera A privacy mask are complete; deliberate-movement recovery and fail-closed acceptance were passed by the mask.
> Phase 2.4 is complete with partial acceptance. After consistently poor performance in earlier formal runs, only the ESP microcontroller node on the ESP-A2 sensor board was replaced on 5 August. In the 6 August 90-minute rerun, a pass was recorded for ESP-A1 at 98.185%, and an improvement from 91.056% to 97.963% was recorded for ESP-A2, although the unchanged 98% target was missed by two packets. The 2 August Attempt 4 result is retained as historical comparison evidence, and the failed latest ESP-A2 measurement is not rewritten as a pass.
> 
> Controlled Phase 5.2 tracking and Phase 5.4 live masked tracking-and-pose acceptance have been passed, with close-overlap ID swaps and an approximately 7 FPS effective processing rate recorded as limitations.
>
> **Phase 6 is complete:** its deterministic SGT alignment, multimodal windows, fusion features, explicit missingness and simultaneous non-research timing validation have been accepted.
> 
> **Phase 7 is complete:** Phase 7.1 is complete with versioned session-manifest, annotation, prediction and incident schemas, role-neutral annotation rules and validation tests.
> **Phase 7.2 is complete:** its automated preflight checks, SGT session IDs, initial manifests and the fail-closed writer-start gate are implemented, tested and accepted on site. Phase 7.3 pause, exclusion, incident, recovery and clean-stop controls are complete, including writer-owned final accounting and schema-enforced privacy-incident rules. Phase 7.4 was passed in an end-to-end non-research run with deliberate node, stream and privacy-mask failures, verified recovery and a clean stop. The recovered mask was visually verified.
> 
> **Phase 8 and Parts 8.1-8.4 are complete; GO was recorded by the Student Researcher on 2 August 2026.**
> 
> **Phases 9, 10 and 11 are complete:** The nine retained formal sessions were approved and all 1,996 source annotations were validated in Phase 10. During cross-participant checking, 461 windows were identified in which one local track was assigned to multiple pseudonyms; both sides were excluded rather than identity being inferred. Together with source uncertainty and track-absence controls, 26,114 of 28,139 participant windows were retained as usable. A total of 26,114 schema-valid rows and 1,847 episodes are included in the audited dataset. In Phase 10.3, 24,046 rows were frozen into eight leave-one-session-out development folds and 2,068 rows into an independent external test. The baselines were established in Phase 11.1, and Camera-only was not exceeded by any early-fusion candidate in Phase 11.2 at 0.442791 development macro F1. The frozen Camera-only model was evaluated once on all external rows: accuracy was measured at 0.492263, balanced accuracy at 0.328030 and macro F1 at 0.348179. Only 19 external rows are included for Reaching/Handling, and F1 was measured at 0.000000. No post-result tuning is permitted. Lower development macro F1 than Camera-only was confirmed for every sensor-only or early-fusion alternative in the formal ablation. All six synthetic missing-modality scenarios were passed: operation is continued with Camera-only during sensor loss, prediction is made unavailable during camera loss, and stale output is not reused. Phase 2 is retained as explicitly partial because the unchanged 98% endurance target was not reached by ESP-A2.
>
> **Phase 12 is complete:** The local inference contract is frozen by `zone-a-phase12.1-local-inference-v1` around the unchanged Camera-only artifact. The frozen artifact is verified and loaded by the service, the ordered 23-feature offline matrix is reproduced, tracks are isolated across completed windows, and schema-valid local JSONL records are delivered idempotently. No forbidden output fields were found during synthetic privacy acceptance, and durable end-to-end delivery was measured at 5.720 ms p95 across 1,000 iterations. Localhost-only operation, a schema-valid non-identifying state boundary, fixed health/staleness rules and the absence of a video component or route are pinned by the Phase 12.2 dashboard contract `zone-a-phase12.2-dashboard-v1`. The real frozen model is used by its state aggregator, local Dash interface and in-memory end-to-end integration path. All 14 synthetic contract scenarios, eight privacy checks and the ready, degraded and privacy-blocked browser captures were passed. Required Firebase Realtime Database integration is frozen by the Phase 12.3 contract `zone-a-phase12.3-firebase-v1` around non-identifying prediction summaries, restricted rules, durable retry and continued local operation during cloud failure. The privacy projector, durable outbox, immutable uploader and isolated inference/dashboard callback are implemented. All ten Rules Emulator scenarios were passed, the restricted rules were deployed, and the guarded synthetic live write/read/delete and unauthenticated-denial checks were passed without retained cloud test data.
>
> **Phase 13 is operationally complete with an accepted limitation and remains PARTIAL overall:** Parts 13.1, 13.2 and 13.3 were passed under `zone-a-phase13-final-acceptance-v2`. A total of 5,400.008 uninterrupted seconds were completed in the final-build run, with clean shutdown and all six metrics recorded. Sustained degradation and A1/A2 delivery of 91.703704%/91.500000% are retained under the explicitly accepted `phase2.4-public-network-delivery` limitation; public Wi-Fi is not claimed as the proven sole cause. The release is frozen as a deterministic source-tree revision with exact required-artifact hashes, privacy scans and a synthetic clean-directory demonstration. Matching Phase 2, the phase is not represented as an unconditional pass.
...
### Completed — through 26 July 2026
The items in this subsection are historical milestones from the superseded three-node design:
- Final three-node hardware split selected
- ESP-A assembled and individually tested
- ESP-B assembled and individually tested
- ESP-T assembled and individually tested
- ESP-A ultrasonic voltage divider established
- MAX4466 numerical audio-feature approach established
- Node-specific UDP JSON telemetry demonstrated
- Node-level sensor values verified
- Hardware photographs and wiring documentation added
- Final repository directory structure established

### Completed — 27 July 2026
- Site permission granted for BatamFast, Singapore Cruise Centre (Harbourfront)
- Participant Ethics Pack Version 4.1 finalised
- UDP receiver implemented
- Packet parser implemented
- Telemetry schema and schema-validation tests added
- Node monitor and associated unit tests added
- Simulated three-node telemetry utility added
- Integration-report generator placeholder added
- Camera manager implemented
- Privacy-mask module implemented
- Privacy-mask procedure documented
- Privacy-mask unit suite completed with 8 passing tests
- detector, tracker, pose-estimator, feature-writer, and vision-pipeline module foundations added
- Development and example mask configurations added

### Completed — 28 July 2026
- Firmware version `1.0.0` added to ESP-A, ESP-B, and ESP-T telemetry
- Node-specific sensor configuration identifiers added to firmware, schema, simulation, ingestion, tests, and documentation
- Boot identifiers added to all node packets
- Receiver restart detection separated from out-of-order packet detection

### Completed — 29 July 2026
- Active deployment narrowed to Zone A
- ESP-A1 and ESP-A2 firmware version `1.1.0` created and compiled
- receiver, schema, simulator and tests restricted to ESP-A1 and ESP-A2
- DJI Camera A-only configuration and MediaMTX path validated
- Ethics Pack Version 4.2 created with Part D and verified as ODT and PDF before being superseded
- Two-board UDP software mock completed with separate node logs
- Ethics Pack Version 4.3 ODT created with an amended Part D in which the Station Manager role is removed and only Counter Agent activity classes are retained
- Zone A calibration protocol updated for ESP-A1, ESP-A2 and DJI Camera A
- Current unit suite expanded to 60 passing tests

### Completed — 30 July 2026
- Participant-facing Ethics Pack Version 4.3 PDF generated and visually verified
- Version 4.3 issued to participating Counter Agents and amended Part D acknowledgements obtained under secure controls
- ESP-A1 and ESP-A2 were initially flashed with firmware version `1.1.0`, then updated to current `1.1.1` firmware used by accepted site evidence
- DJI Camera A wireless RTMP ingest and local video-only RTSP relay verified at 1920×1080 and approximately 30 FPS
- Short-run thresholds passed in a 60-second combined ESP-A1/ESP-A2/Camera A diagnostic
- Final ESP-A1 and ESP-A2 staff-side placements documented and accepted as physically safe and ready for Phase 2 calibration
- Phase 2.3 site sensor calibration passed, with intended and cross-placement targets met at both PIR placements, all three ESP-A1 ultrasonic reference points passed, and usable numerical-audio separation recorded for both nodes
- Phase 2.4 Attempt 3 completed for 90 minutes with partial acceptance: ESP-A1 passed at 98.294%, while ESP-A2 recorded 93.628% on uncontrolled public Wi-Fi network and remained below unchanged 98% target
- Polygon exclusion support added to privacy mask with BatamFast draft retained as `verified: false`
- Phase 2 telemetry evidence generator and controlled-trial templates added
- Unit suite expanded to 83 passing tests
- Consent and acknowledgement records moved to secure storage outside repository
- Repository ignore rules reviewed and approved
- Zone A data-handling procedure reviewed and approved

### Completed — 31 July 2026
- Phase 3.4 Camera A endurance passed: headless 1920×1080 diagnostic completed 90 minutes at 29.969 measured FPS with 161,836 successful reads and no read failures
- Sequence delivery of 100% achieved by both ESP-A1 and ESP-A2 during the Camera A endurance window
- Local video-only H264 path recovered automatically after a controlled DJI publisher interruption without either ESP board being disrupted
- Versioned Camera A frame/track records, component latency fields and explicit frame/pose availability added
- Masked-only Camera A preview and interactive polygon calibration completed on site, closing Phases 4.2 and 4.3
- Final Camera A mask version `batamfast-v1` verified after deliberate movement, fail-closed recovery and a live masked-pipeline acceptance run, closing Phase 4.4; this historical version was superseded by visually verified three-polygon `batamfast-v2` freeze on 4 August
- Phase 5 unit suite expanded to 112 passing tests
- Phase 5.2 camera-local tracking and Phase 5.4 live masked tracking-and-pose acceptance passed, with final performance and identity limitations recorded in `docs/testing/results/`
- Phase 6 versioned two-second SGT windows, fusion-ready Camera A/ESP-A1/ESP-A2 features, deterministic rebuilding and explicit missing-modality handling implemented, expanding unit suite to 120 passing tests
- Phase 7.1 versioned collection schemas and role-neutral annotation protocol completed, expanding unit suite to 152 passing tests
- Phase 7.3 pause, exclusion, incident, recovery and writer-owned clean-stop accounting controls completed, with complete unit suite at 224 passing tests

### Completed — 1 August 2026
- Phase 6.1 live simultaneous timing evidence accepted, completing Phase 6
- Phase 7.2 live fail-closed preflight and manifest-before-writer evidence accepted
- Phase 7.4 end-to-end non-research dry run completed with deliberate ESP-A1, Camera A stream and privacy-mask incidents, verified recovery, deletion for the privacy interval and clean writer-owned stop accounting
- Recovered `camera_a` privacy mask was visually verified before resume
- Output counts, schemas and prohibited identity/media scan passed
- A total of 246 passing tests are recorded for the complete unit suite

### Completed — 2 August 2026
- Separate ESP-A1 and ESP-A2 restart recovery trials passed on the site's public network, with packets continuously sent by the other active board
- Phase 2.4 Attempt 4 completed full 90-minute combined interval from 14:13:15 to 15:43:15 SGT
- A total of 5,201 / 5,400 packets (96.315%) delivered by ESP-A1 and 4,917 / 5,400 packets (91.056%) delivered by ESP-A2, both below the unchanged 98% target
- Delivery failures were accepted as a known operational limitation of uncontrolled public network without being rewritten as passing results
- The same 5,400-second interval completed by Camera A at 1920x1080 with no read failures; its 29.476 measured FPS, 2.914-second longest gap and diagnostic drop estimate retained as combined-run performance anomalies
- Phase 8.3 closed by the completed combined run, with failed delivery targets retained as accepted operational limitations
- Accepted placements, sensor calibration, verified mask, packet and recovery results, and Camera A health consolidated in the dated Phase 8.1 live-site calibration record, with all operational limitations retained
- Fail-closed configuration, movement, stream-loss, exclusion, deletion and verified-recovery evidence consolidated as a pass in the dated Phase 8.2 privacy-failure and recovery record

### Completed — 3-4 August 2026
- Phase 9.1 completed with approved corrective pilot, lifecycle-owned browser annotation controls and automatic two-second SGT boundaries
- A present selected track referenced by all nine corrective-pilot annotations throughout all 176 associated windows
- Accepted 87.257% pose availability and below-target ESP delivery remain explicit limitations rather than rewritten passes
- Revised verified three-polygon mask is versioned as `batamfast-v2`
- Phase 9.2 completed with a hash-locked collection profile, session-manifest schema version 4 and fail-closed formal-setting enforcement
- Documentation reconciliation superseded by `zone-a-collection-v3`, in which pose weights and critical application source files are also locked and live ESP firmware version 1.1.1 is enforced; the superseding profile is recorded in session-manifest schema version 5 without version 4 being silently changed
- Participant-card annotation UI accepted after the first formal run and frozen as `zone-a-collection-v4`; the new active profile is recorded in session-manifest schema version 6 without completed v3 data being reinterpreted
- `Serving` and `Typing/Processing` merged in the post-run three-class technical candidate; annotation/prediction schema version 3 and manifest schema version 8 used, consent-gated audited participant-card additions supported during a run, and v2 historical records and mappings preserved
- Desktop split layout accepted on 5 August 2026: complete masked display and annotation tools retained on the left, participant cards positioned at the top right, and revised source frozen by `zone-a-collection-v6` without the Version 4.4 method, activity classes or schemas being changed
- Polling-safe annotation controls accepted after manually entered input was cleared through card reconstruction in the third formal attempt; manual track drafts, focus and selected activities are preserved by `zone-a-collection-v7` across ordinary one-second status polls, with fail-closed replacement-track prompts retained
- Retired ESP-A/ESP-B/ESP-T root telemetry was moved out of `data/raw` into `data/historical/telemetry_progression/`
- Failed first annotation-control pilot was moved to `data/quarantine/phase_9_1_annotation_control_failure/`, while abandoned never-started manifest was deleted

### Completed — 5-6 August 2026
- Only ESP microcontroller node on ESP-A2 sensor board was replaced on 5 August after consistently poor performance in earlier formal runs
- 6 August formal 90-minute rerun is current Phase 2.4 endurance evidence, while 2 August Attempt 4 result remains historical comparison evidence
- A pass recorded for ESP-A1 at 5,302 / 5,400 packets (98.185%); an improvement recorded for ESP-A2 to 5,290 / 5,400 packets (97.963%), with the unchanged target still missed by two packets
- Phase 2.4 is complete with partial acceptance. 2 August ESP-A2 recovery trial predates microcontroller replacement and remains supplemental evidence with that limitation

### Completed — 10 August 2026
- Phase 9.3 completed with nine retained formal participant sessions and clean completed manifests
- A total of 8 hours 00 minutes 20 seconds of wall-clock collection recorded across retained sessions
- A total of 14 hours 59 minutes 40 seconds of usable labelled data produced through concurrent participant annotation across 26,990 two-second participant windows
- Derived three-class balance is 63.45% Serving/Processing, 31.66% Idle/Waiting and 4.89% Reaching/Handling
- Nine pseudonymised participants are represented, with uncertain and excluded intervals retained as explicit quality evidence rather than silently relabelled
- Resolved first-run privacy incident and measured ESP-A2 delivery and multi-person CV throughput limitations remain part of final collection record
- Original `zone-a-collection-v7` checksum `2efda050...` retained in Runs 3-7, while revised checksum `87758c7a...` is recorded in Runs 8-9 after comment and documentation wording was revised without runtime settings being changed
- Phase 9 is complete. Phase 10 dataset reconstruction, privacy and quality audit, participant-level feature building and grouped split freezing were scheduled next

### Completed — 20 August 2026
- Exactly nine completed formal participant sessions selected in Phase 10.1 and approved by the project owner, with pilots, dry runs, the aborted session and the completed session with no annotations excluded
- All selected manifest output counts reconciled with their physical files and declared exclusions
- All 1,996 source annotations validated against their versioned schema, with original records preserved and historical labels mapped only in derived output
- A total of 13 terminal selected-track-absent windows identified across five sessions through deterministic track/window checking; 11 previously marked usable are now explicitly excluded
- A total of 28,139 participant-window annotation rows and 26,114 usable three-class rows included in the normalised result after track-absence and conflicting-assignment exclusions
- Phase 10.1 source approval and annotation normalisation are complete
- A total of 26,114 rows and 1,847 activity episodes produced through the Phase 10.2 participant-specific feature schema and deterministic builder
- Phase 10.2 data-quality and privacy audits passed, with 2,025 exclusions fully reconciled and no remaining track-assignment, schema, non-finite, identity, role, media or retired-component failure
- Phase 10.2 is complete
- A total of 24,046 development rows frozen across eight leave-one-session-out folds and 2,068 rows frozen in an independent participant-and-session-isolated external test in Phase 10.3
- All 26,114 rows reconciled exactly, with assignment, schema, participant, session, episode and temporal-window leakage checks passed
- Phase 10 is complete. Phase 11.1 development-only baseline result is recorded below

### Completed — 22 August 2026
- Seed 3070, three Random Forest candidates, class order, modality roots and Phase 10 source hashes locked in `zone-a-phase11.1-baselines-v1` in Phase 11.1
- Fold-local median imputation and missingness indicators are fitted only on each training fold
- Out-of-fold predictions provided for all 24,046 development rows across eight frozen leave-one-session-out folds
- Development macro F1 is 0.260477 for majority, 0.442791 for Camera A-only, 0.368823 for ESP-A1-only and 0.369954 for ESP-A1+ESP-A2
- Same 100-tree, depth-12, minimum-leaf-1 balanced-subsample Random Forest candidate is selected for all three learned baselines
- 2,068-row external test remains unevaluated and reserved for one final Phase 11 evaluation
- Phase 11.1 is complete
- `zone-a-phase11.2-multimodal-v1` frozen in Phase 11.2, with accepted Phase 11.1 metrics used as the comparison floor
- Out-of-fold predictions provided by Camera+A1, Camera+A2 and Camera+A1+A2 early-fusion candidates for all 24,046 development rows across the same eight folds
- Development macro F1 is 0.406241 for Camera+A1, 0.404674 for Camera+A2 and 0.390878 for Camera+A1+A2
- Camera+A1 is strongest multimodal candidate, but it remains 0.036551 below Camera-only, so Camera-only remains overall development choice
- All Phase 11.2 metrics, fitted models and model cards rebuilt byte-identically, with the external test left untouched
- Phase 11.2 is complete
- Phase 11.3 evaluation contract frozen and synthetic tests passed by its guarded evaluator
- Every frozen hash and exactly 2,068 locked external assignments verified during real-data preflight, with zero feature rows loaded for prediction and no evaluation output written
- One-time Phase 11.3 Camera-only external evaluation is complete across all 2,068 locked rows
- External accuracy is 0.492263, balanced accuracy is 0.328030 and macro F1 is 0.348179
- Only 19 external rows included for Reaching/Handling and F1 measured at 0.000000, retained as a strong limitation
- Accepted aggregate-only metrics are frozen and overwrite protection is active
- Phase 11.3 development ablation table and figure are complete using frozen development evidence only
- All six synthetic scenarios passed for Phase 11.3 missing-modality behaviour without participant data or a repeated external evaluation
- Phase 11.3 is complete with the external Reaching/Handling limitation retained
- Phase 12.1 local inference contract `zone-a-phase12.1-local-inference-v1` is frozen with SHA-256 `68fa975e859a386c32d991557a88303b1f403f23b5886ff4dbf5093c49befba8`
- The unchanged Camera-only model, per-track two-second preprocessing, 23 ordered features, schema hashes, class/confidence semantics, degraded modes and local privacy boundary are pinned by the contract
- The frozen feature structure is reproduced per local track by the shared Phase 12 preprocessing adapter, cross-track aggregation is rejected, and camera-unavailable windows are preserved without the hash-pinned historical collection module being changed
- The contract, model and schema hashes are verified by the fail-closed service core; the unchanged Camera-only pipeline is loaded; deterministic schema-valid predictions are emitted; and camera-loss or model-error outputs are cleared without stale state being retained
- Every Camera A local track is isolated through completed-window orchestration, an explicit unavailable record is emitted for a window with no track, and deterministic predictions are delivered to a restart-safe idempotent local JSONL log
- Only explicitly declared `synthetic` or `non_research` numerical/track inputs are accepted by `scripts/run_local_inference.py`; participant and pilot input use is prohibited
- Phase 12.1 synthetic privacy acceptance passed without participant data, external-test access, Firebase, retained media, raw features or forbidden identifying fields
- Durable end-to-end local JSONL delivery measured at 5.720 ms p95 across 1,000 iterations, below the two-second feature-window duration
- Phase 12.1 and Phase 12.2 are complete; dashboard contract `zone-a-phase12.2-dashboard-v1` remains frozen at SHA-256 `1fcd1343f854691e2bce3fabb22b4f9b6f2b048a6d94e9406a7d31be1dcaca76`
- Only local prediction summaries, Camera A health, ESP-A1/A2 health, privacy readiness and fixed warning codes are permitted by dashboard state schema v1; session/participant identity, arbitrary warning text, raw features, audio and video are excluded
- The dashboard is pinned to `127.0.0.1:8050`, one-second refresh, unchanged numerical model confidence, explicit stale/unavailable state, verified-mask precedence and no persisted dashboard snapshots
- All pinned hashes are verified by the dashboard state aggregator, schema-valid prediction callbacks are accepted, only the latest completed window is retained, older track activity is cleared on a newer unavailable window, and schema-valid snapshots are emitted without session or model metadata
- Independent in-memory ESP-A1/A2 received-packet, sequence-gap and estimated-loss counters are added by the Phase 12.2 aggregator alongside the unchanged historical node monitor; warning codes are created from fixed thresholds without arbitrary diagnostic text being exposed
- Camera-local activity, unchanged model confidence, explicit stale/unavailable state, Camera A/mask health, ESP-A1/A2 freshness and packet loss, and fixed warning codes are displayed by the local Dash interface without a media component or route
- Metadata-only Camera A and active ESP receive records are validated by `src/aria/dashboard/integration.py`, health summaries are forwarded, and the Phase 12.1 prediction callback is connected; raw camera/sensor features are never retained and a dashboard log is never created
- The dashboard is started only on the frozen `127.0.0.1:8050` boundary by `PYTHONPATH=src python3 scripts/run_local_dashboard.py`; `--input-scope synthetic` or `--input-scope non_research` is additionally required with `--demo`, and Camera A metadata, ESP-A1/A2 envelopes, the real frozen model and dashboard state are now exercised entirely in memory
- All 14 synthetic dashboard scenarios, eight privacy checks and three browser-rendered safety-state captures passed; no participant data, external test data or Firebase was used
- A total of 32 passes recorded for the focused Phase 12.2 contract, state, UI, integration and acceptance gate; 429 passes and the same three pre-existing frozen-hash failures recorded for the complete unit suite
- Phase 12.3 is complete with required Firebase Realtime Database contract `zone-a-phase12.3-firebase-v1` frozen at SHA-256 `48e3cb907c7586e43dcf9bc956331408df557c739c2e1f581b55e30b2bbb2afb`
- Only non-identifying prediction summaries are permitted at the versioned `aria/schema_version_1` path by the Phase 12.3 contract; session, window, track and participant identifiers, feature vectors, audio and all video are excluded
- Records must be queued locally during Firebase failure, and inference and the dashboard must not be blocked; cloud retraining and model-weight updates are prohibited by the frozen Phase 11 result
- The full local record is validated by the Phase 12.3 projector, only the 12-field maximum cloud schema is emitted, conditional nulls are omitted, and invalid or identifying sources are rejected before the outbox
- Records are written atomically by the durable outbox, retries are performed oldest-first with bounded backoff, identical retries are treated idempotently, immutable conflicts are preserved, and records are deleted only after a verified Firebase transaction
- Firebase queuing is performed through an isolated non-throwing inference callback, and network delivery is run separately, so local inference and dashboard state are kept operational during cloud failure
- The safe local queue is exercised without cloud access by `PYTHONPATH=src python3 scripts/sync_firebase_predictions.py --input <predictions.jsonl> --input-scope synthetic --queue-only`; `--queue-only` must be omitted only after rules and credentials are ready
- A total of 20 passes recorded for the focused Phase 12.3 contract, local uploader and guarded live-acceptance gate
- Ten synthetic scenarios passed by the Realtime Database Rules Emulator using local-only project `demo-aria-phase12-3`: valid available/unavailable writes, unauthorized denial, immutability, forbidden-field rejection, conditional schema enforcement, frozen values, synthetic acceptance write/delete and versioned-path isolation
- Emulator evidence is recorded at `outputs/acceptance/phase_12_3/rules_emulator.json`; no live Firebase, participant data or external-test data was accessed
- The emulator-tested restricted rules were deployed only to `aria-e906f-default-rtdb`; the guarded synthetic live write/read/delete check was passed and deletion was verified, while an unauthenticated live read was denied with HTTP 401
- All twelve frozen scenarios recorded as passed in aggregate Phase 12.3 evidence without participant or external-test data; Phase 12.3 and Phase 12 are complete
